...
@app.callback(
    ...
    [Input('yaxis-type-us', 'value')])
def update_indicator_graphic_us(yaxis_type_us):
...
@app.callback(
    ...
    [Input('yaxis-type-state', 'value'), Input('state-selector', 'value')])
def update_indicator_graphic_state(yaxis_type_us, state):
    start_date = pd.Timestamp("2020-03-01")
